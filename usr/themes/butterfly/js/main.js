        ...
        if (commentForm) {
            ...
            commentForm.addEventListener('submit', function(e) {
                ...
                requiredFields.forEach(field => {
                    if (!field.value.trim()) {
                        console.error(`必填字段 ${field.name} 为空`);
                        allFieldsFilled = false;
                    }
                });
                
                if (!allFieldsFilled) {
                    console.error('表单验证失败：某些必填字段为空');
                    e.preventDefault();
                    alert('请填写所有必填字段');
                    return false;
                }
            ...
            console.log('未找到评论表单 #comment-form');
        }
    });
    ...
    try {
        ...
        lazyImages.forEach(function(img) {
            ...
            if (src) {
                ...
                // 对特定图床的图片设置最高质量
                if ((src.includes('s2.loli.net') || 
                     src.includes('awtc.pp.ua') || 
                     src.includes('loli.net') || 
                     src.includes('pp.ua')) && 
                    !src.includes('quality=100')) {
                    
                    if (src.includes('quality=')) {
                        // 替换已有的质量参数
                        optimizedSrc = src.replace(/quality=\d+/, 'quality=100');
                    } else {
                        // 添加质量参数
                        const separator = src.includes('?') ? '&' : '?';
                        optimizedSrc = src + separator + 'quality=100';
                    }
                    img.setAttribute('data-lazy-src', optimizedSrc);
                }
                
                // 设置原生加载属性，提高浏览器原生懒加载支持
        ...
        if ('IntersectionObserver' in window) {
                ...
                entries.forEach(function(entry) {
                    if (entry.isIntersecting) {
                        ...
                        if (src) {
                            ...
                            lazyImage.style.imageRendering = 'high-quality';
                            lazyImage.style.transform = 'translateZ(0)';
                            lazyImage.style.backfaceVisibility = 'hidden';
                            ...
                            // 移除不必要的属性
                            lazyImage.removeAttribute('data-ll-status');
            ...
            console.log('使用 IntersectionObserver 初始化懒加载');
        } else {
            // 降级处理：对于不支持IntersectionObserver的浏览器
            console.log('浏览器不支持 IntersectionObserver，降级处理懒加载');
        ...
        setTimeout(function() {
            lazyImages.forEach(function(img) {
                if (img.src.includes('data:image/gif') || !img.classList.contains('loaded')) {
                    const src = img.getAttribute('data-lazy-src');
                    if (src) {
                        console.log('修复未加载的图片:', src);
                        img.src = src;
        ...
        try {
            ...
            console.error('紧急加载图片时出错:', fallbackError);
        }
    }
}
