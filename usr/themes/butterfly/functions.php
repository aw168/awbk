...
function commentRank($widget, $email = NULL, $justReturn = false)
{
    if (empty($email) && !empty($widget)) {
        $email = $widget->mail;
    }
    ...
    if (!empty($email)) {
        $db = Typecho_Db::get();
        $userInfo = $db->fetchRow($db->select('group')
                ->from('table.users')
                ->where('mail = ?', $email));
        ...
        if (!empty($userInfo)) {
            ...
            // 非注册用户，判断是否为博主设置的常用邮箱
            if (!empty(Helper::options()->customMail) && in_array($email, explode(',', Helper::options()->customMail))) {
                $rank = 'vmaster';
                $rankName = '博主';
            }
        }
    ...
    // 输出标签
    echo '<span class="vtag ' . $rank . '">' . $rankName . '</span>';
}
...
{
    $db = Typecho_Db::get();
    $parentID = $db->fetchRow($db->select('parent,author,status')
            ->from('table.comments')
            ->where('coid = ?', $coid))['parent'];
    
    if ($parentID !== "0") {
        $author = $db->fetchRow($db->select('author,status,mail')
                ->from('table.comments')
                ->where('coid = ?', $parentID));
                
        if (!empty($author) && $author['status'] == "approved") {
            $parentAuthor = $author['author'];
            $parentMail = $author['mail'];
            
            // 根据邮箱判断是否为博主
            $isAuthor = commentRank(null, $parentMail, true) == 'vmaster';
            $authorClass = $isAuthor ? 'author-name' : '';
            
            return '<span class="parent-icon"><i class="fas fa-share"></i></span> 回复给 <a href="#comment-' . $parentID . '" class="parent-comment-link ' . $authorClass . '">' . $parentAuthor . '</a>';
        }
    }
    ...
    <li id="li-<?php $comments->theId(); ?>" class="comment-body<?php
        ...
        <div id="<?php $comments->theId(); ?>">
            <div class="comment-author">
                ...
                <div class="comment-main">
                    <div class="comment-meta-top">
                        <div class="vuser">
                            <a href="<?php $comments->url(); ?>" target="_blank" rel="nofollow" class="vnick" data-user-id="<?php $comments->coid(); ?>">
                                <?php $comments->author(); ?>
                            </a>
                    ...
                    <div class="vhead">
                        <div class="comment-meta">
                            ...
                            <?php endif; ?>
                            <span class="vtime" title="<?php $comments->date('Y-m-d H:i:s'); ?>"><?php $comments->date('Y-m-d H:i'); ?></span>
                            <?php if (Helper::options()->CloseComments == 'off') : ?>
                                <span class="comment-reply">
                                    <?php $comments->reply('<i class="fas fa-reply"></i>'); ?>
                                </span>
                            <?php endif ?>
                        </div>
                    </div>
            ...
            <div class="comment-ua">
                <i class="fas fa-desktop"></i> <?php getOs($comments->agent); ?>
                <i class="fas fa-globe ml-2"></i> <?php getBrowser($comments->agent); ?>
            </div>
        </div>
...
function loadCommentsAssets() {
    if (Helper::options()->commentsStyle !== 'off') {
        echo '<link rel="stylesheet" href="' . Helper::options()->themeUrl . '/css/comments.css">';
        echo '<link rel="stylesheet" href="' . Helper::options()->themeUrl . '/css/comments-form.css">';
        echo '<script src="' . Helper::options()->themeUrl . '/js/comments.js"></script>';
    }
}
