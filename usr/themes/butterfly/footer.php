    ...
    <footer id="modern-footer">
        <div class="footer-glass-effect">
            <div class="footer-content">
                ...
                <?php if (is_array($this->options->beautifyBlock) && in_array('showFooterSiteLinks', $this->options->beautifyBlock)): ?>
                <div class="site-links">
                    <a href="<?php $this->options->feedUrl(); ?>" class="hover-underline site-link" target="_blank">
                        <i class="fas fa-rss"></i> RSS订阅
                    </a>
                    <a href="<?php $this->options->siteUrl(); ?>sitemap.xml" class="hover-underline site-link" target="_blank">
...
<!-- 图片质量优化脚本 -->
<script src="<?php $this->options->themeUrl('js/image-handler.js'); ?>?v=1.0.1"></script>
<script src="<?php $this->options->themeUrl('js/image-fix.js'); ?>?v=1.0.1"></script>

<!-- 评论加载优化脚本 -->
<script src="<?php $this->options->themeUrl('js/comments-cache.js'); ?>?v=1.0.0"></script>

<!-- OwO表情选择器 -->
<script src="<?php $this->options->themeUrl('js/OwO.min.js'); ?>?v=1.0.0"></script>
<link rel="stylesheet" href="<?php $this->options->themeUrl('css/OwO.min.css'); ?>?v=1.0.0">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function enhanceImages() {
        // 为文章中的所有图片添加高质量属性
        document.querySelectorAll('.article-content img, .post-content img').forEach(function(img) {
            img.classList.add('high-quality-img');
            if (img.complete && img.hasAttribute('data-lazy-src')) {
                // 确保已加载的图片使用高质量源
                const highQualitySrc = img.getAttribute('data-lazy-src');
                if (highQualitySrc && img.src !== highQualitySrc && !img.src.includes('404.jpg')) {
